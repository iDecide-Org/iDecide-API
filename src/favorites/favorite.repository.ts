...
export class FavoritesRepository {
  ...
  // --- University Methods ---
  async addFavoriteUniversity(userId: string, universityId: string): Promise<FavoriteUniversity> {
    const favorite = this.favUniversityRepository.create({ userId, universityId });
    try {
      await this.favUniversityRepository.save(favorite);
      // Re-fetch with relation to return the full university object
      return this.favUniversityRepository.findOne({ where: { id: favorite.id }, relations: ['university'] });
    } catch (error) {
  ...
  async removeFavoriteUniversity(userId: string, universityId: string): Promise<void> {
    const result = await this.favUniversityRepository.delete({ userId, universityId });
    if (result.affected === 0) {
      throw new NotFoundException('Favorite university not found.');
    }
  }

  async findUserFavoriteUniversities(userId: string): Promise<FavoriteUniversity[]> {
     try {
  ...
  // --- Scholarship Methods ---
  async addFavoriteScholarship(userId: string, scholarshipId: string): Promise<FavoriteScholarship> {
    const favorite = this.favScholarshipRepository.create({ userId, scholarshipId });
    try {
      await this.favScholarshipRepository.save(favorite);
      // Re-fetch with relation to return the full scholarship object
      return this.favScholarshipRepository.findOne({ where: { id: favorite.id }, relations: ['scholarship'] });
    } catch (error) {
      if (error.code === '23505') { // Unique constraint violation
        throw new ConflictException('Scholarship already in favorites.');
      }
      console.error("Error adding favorite scholarship:", error);
      throw new InternalServerErrorException('Failed to add favorite scholarship.');
    }
  }

  async removeFavoriteScholarship(userId: string, scholarshipId: string): Promise<void> {
    const result = await this.favScholarshipRepository.delete({ userId, scholarshipId });
    if (result.affected === 0) {
      throw new NotFoundException('Favorite scholarship not found.');
    }
  }

  async findUserFavoriteScholarships(userId: string): Promise<FavoriteScholarship[]> {
     try {
      ...
      throw new InternalServerErrorException('Failed to retrieve favorite scholarships.');
    }
  }
}
