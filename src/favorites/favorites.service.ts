...
export class FavoritesService {
  ...
  // --- University Methods ---
  async addUniversityToFavorites(
    userId: string,
    universityId: string,
  ...
  async removeUniversityFromFavorites(
    userId: string,
    universityId: string,
  ): Promise<void> {
    await this.favoritesRepository.removeFavoriteUniversity(
      userId,
      universityId,
    );
  }

  async getUserFavoriteUniversities(
    userId: string,
  ): Promise<FavoriteUniversity[]> {
  ...
  // --- Scholarship Methods ---
  async addScholarshipToFavorites(
    userId: string,
    scholarshipId: string,
  ): Promise<FavoriteScholarship> {
    return this.favoritesRepository.addFavoriteScholarship(
      userId,
      scholarshipId,
    );
  }

  async removeScholarshipFromFavorites(
    userId: string,
    scholarshipId: string,
  ): Promise<void> {
    await this.favoritesRepository.removeFavoriteScholarship(
      userId,
      scholarshipId,
    );
  }

  async getUserFavoriteScholarships(
    userId: string,
  ): Promise<FavoriteScholarship[]> {
